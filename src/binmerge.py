...
def binmerge(cuefile, name, output_directory):
  cue_map = read_cue_file(cuefile)
  cuesheet = gen_merged_cuesheet(name+'.bin', cue_map)

  if not os.path.exists(output_directory):
    print("Output dir does not exist")
    return False

  with open(os.path.join(output_directory, name+'.cue'), 'w', newline='\r\n') as f:
    f.write(cuesheet)
  print("Wrote %s" % name+'.cue')

  print("Merging files...")
  if merge_files(os.path.join(output_directory, name+'.bin'), cue_map):
    print("Wrote %s" % name+'.bin')
  else:
